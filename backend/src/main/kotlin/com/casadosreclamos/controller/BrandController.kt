...
class BrandController {
    ...
    @Path("/obsolete/{brand}")
    @RolesAllowed(MANAGER_ROLE, ADMIN_ROLE)
    @Operation(summary = "Make a brand obsolete")
    @APIResponses(
        APIResponse(responseCode = "200", description = "Successful operation"),
        APIResponse(responseCode = "401", description = "User is not logged in"),
        APIResponse(responseCode = "403", description = "Insufficient privileges")
    )
    fun makeObsolete(@PathParam("brand") brand: Long): Uni<Response> {
        return identity.deferredIdentity.onItem().transformToUni { id ->
            logger.info("User ${id.principal.name} is making brand $brand obsolete")

            return@transformToUni brandService.makeObsolete(brand).onItem().transform { _ ->
                Response.ok().build()
            }
        }
    }
