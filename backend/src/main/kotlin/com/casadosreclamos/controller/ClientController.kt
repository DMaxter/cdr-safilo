...
class ClientController {
    ...
    @Inject lateinit var identity: CurrentIdentityAssociation
    ...
    fun getAllClients(): Multi<ClientDto> {
        return identity.deferredIdentity.onItem().transformToMulti { id ->
            ...
            return@transformToMulti clientService.getAll().onItem().transform { ClientDto(it) }
        }
    }
    ...
    @Path("/{id}")
    @Authenticated
    @Operation(summary = "Obtain a specific client")
    @APIResponses(
            APIResponse(responseCode = "200", description = "Client obtained"),
            APIResponse(responseCode = "401", description = "User is not logged in")
    )
    fun getClient(@PathParam("id") clientId: Long): Uni<ClientDto> {
        return identity.deferredIdentity.onItem().transformToUni { id ->
            logger.info("User ${id.principal.name} is requesting client with id ${clientId}")

            return@transformToUni clientService.get(clientId).onItem().transform { ClientDto(it) }
        }
    }
    ...
    @APIResponses(
            ...
            APIResponse(
                    responseCode = "403",
                    description = "User doesn't have authorization to register a client"
            )
    )
    fun addClient(client: ClientDto): Uni<ClientDto> {
        return identity.deferredIdentity.onItem().transformToUni { id ->
            logger.info(
                    "User ${id.principal.name} is registering client with name \"${client.name}\""
            )

            return@transformToUni clientService.add(client).onItem().transform { ClientDto(it) }
        }
    }
    ...
    @APIResponses(
            ...
            APIResponse(
                    responseCode = "403",
                    description = "User doesn't have authorization to edit a client"
            )
    )
    fun editClient(client: ClientDto): Uni<ClientDto> {
        return identity.deferredIdentity.onItem().transformToUni { id ->
            ...
            return@transformToUni clientService.update(client).onItem().transform { ClientDto(it) }
        }
    }
    ...
    fun importClients(
        ...
        return identity.deferredIdentity.onItem().transform { id ->
            ...
            clientService
                    .importClients(file.uploadedFile().toFile())
                    .onCompletion()
                    .invoke { -> logger.info("Successfully uploaded all clients") }
                    .collect()
                    .asList()
                    .await()
                    .indefinitely()
