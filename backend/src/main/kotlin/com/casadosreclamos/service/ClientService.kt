...
class ClientService {
    ...
    @Inject lateinit var bannerRepository: BannerRepository
    ...
    fun add(clientDto: ClientDto): Uni<Client> {
        ...
        } else if (clientDto.email == null ||
                        clientDto.email!!.isEmpty() ||
                        !EMAIL_REGEX.matches(clientDto.email!!)
        ) {
        ...
        } else if (clientDto.phone == null ||
                        clientDto.phone!!.isEmpty() ||
                        !PHONE_REGEX.matches(clientDto.phone!!)
        ) {
    ...
    fun update(clientDto: ClientDto): Uni<Client> {
        ...
        return Panache.withTransaction {
            clientRepository
                    ...
                    .onItem()
                    .invoke { _ -> logger.info("Client updated successfully") }
                    .onFailure()
    ...
    fun editNote(clientId: Long, note: String): Uni<Client> {
        ...
        return Panache.withTransaction {
            clientRepository
                    ...
                    .onItem()
                    .invoke { _ -> logger.info("Successfully edited note") }
                    .onFailure()
    ...
    fun importClients(file: File): Multi<Uni<Client>> {
        val contents =
                CSVParser.parse(file, StandardCharsets.UTF_8, CSVFormat.EXCEL.withDelimiter(';'))
        val records = contents.records

        logger.info("Importing ${records.size - 1} clients")

        val multi: Multi<Uni<Client>> =
                Multi.createFrom().emitter { em ->
                    ...
                    for (record in records.drop(1)) {
                        ...
                        em.emit(
                                clientRepository.findById(id).onItem().transformToUni { client ->
    ...
    private fun createBannerIfNotExists(id: String): Uni<Banner> {
        return Panache.withTransaction {
            bannerRepository.findById(id).onItem().ifNull().switchTo {
                ...
                bannerRepository
                        .persist(banner)
                        .invoke { _ -> logger.info("Successfully created banner") }
                        .onFailure()
