...
class MaterialService {
    ...
    fun add(materialDto: MaterialDto): Uni<Material> {
        ...
        val finishingGroupUni =
                if (materialDto.mandatoryFinishings.isNullOrEmpty()) {
                    Uni.createFrom().item(setOf())
                } else {
                    finishingService
                            .findGroups(materialDto.mandatoryFinishings!!)
                            .collect()
                            .with(Collectors.toSet())
                }

        val finishingsUni =
                if (materialDto.additionalFinishings.isNullOrEmpty()) {
                    Uni.createFrom().item(setOf())
                } else {
                    finishingService
                            .find(materialDto.additionalFinishings!!)
                            .collect()
                            .with(Collectors.toSet())
                }
        ...
        return Panache.withTransaction {
            Uni.combine()
                    .all()
                    .unis(materialExistsUni, finishingGroupUni, finishingsUni)
                    .asTuple()
                    .onItem()
                    .transformToUni { tuple ->
                        ...
                        if (exists) {
                            logger.error(
                                    "A material with name ${materialDto.name} is already registered"
                            )

                            throw AlreadyExistsException("Material")
                        } else if (materialDto.mandatoryFinishings != null &&
                                        groups.size < materialDto.mandatoryFinishings!!.size
                        ) {
                            logger.error(
                                    "Invalid or repeated finishing groups detected. Fetched: $groups"
                            )

                            throw InvalidIdException("grupo de acabamentos")
                        } else if (materialDto.additionalFinishings != null &&
                                        finishings.size < materialDto.additionalFinishings!!.size
                        ) {
                            logger.error(
                                    "Invalid or repeated additional finishings detected. Fetched: $finishings"
                            )
                        ...
                        materialRepository
                                .persist(material)
                                .onItem()
                                .invoke { _ -> logger.info("Successfully created material") }
                                .onFailure()
    ...
    fun update(materialDto: MaterialDto): Uni<Material> {
        ...
        return Panache.withTransaction {
            materialRepository
                    .exists(materialDto.name!!, materialDto.id!!)
                    .onItem()
                    .transformToUni { exists ->
                        ...
                        Uni.combine()
                                .all()
                                .unis(materialUni, finishingGroupUni, finishingsUni)
                                .asTuple()
                                .onItem()
                                .transform { tuple ->
                                    ...
                                    if (material == null) {
                                        logger.error(
                                                "Material with ID ${materialDto.id} is not registered"
                                        )

                                        throw InvalidIdException("material")
                                    } else if (materialDto.mandatoryFinishings != null &&
                                                    groups.size <
                                                            materialDto.mandatoryFinishings!!.size
                                    ) {
                                        logger.error(
                                                "Invalid or repeated finishing groups detected. Fetched: $groups"
                                        )
                                        ...
                                        throw InvalidIdException("grupo de acabamentos")
                                                    ...
                                                    finishings.size <
                                                            materialDto.additionalFinishings!!.size
                                    ) {
                                        logger.error(
                                                "Invalid or repeated additional finishings detected. Fetched: $finishings"
                                        )
    ...
    fun delete(id: Long): Uni<Response> {
        return Panache.withTransaction { materialRepository.deleteById(id) }
                .onItem()
                .transform { _ ->
