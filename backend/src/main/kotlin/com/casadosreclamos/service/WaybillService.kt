...
class WaybillService {
    ...
    @Inject
    lateinit var client: FemaClient

    @Inject
    lateinit var mailer: ReactiveMailer

    @Inject
    @ConfigProperty(name = "fema.report.emails")
    ...
    fun openWaybill(requestId: Long, waybillDto: WaybillDto): Uni<File> {
        ...
        } else if (waybillDto.dimensions?.length == null || waybillDto.dimensions.length <= 0 || waybillDto.dimensions.height == null || waybillDto.dimensions.height <= 0 || waybillDto.dimensions.width == null || waybillDto.dimensions.width <= 0) {
        ...
        return Panache.withTransaction {
            requestService.get(requestId).onItem().transformToUni { request ->
                ...
                client.openWaybill(waybillDto).onItem().transform { label ->
    ...
    fun closeDay(): Uni<Void> {
        ...
        return client.closeDay().onItem().transformToUni { report ->
            ...
            return@transformToUni mailer.send(
                Mail.withText("", "Fecho diário FEMA", getEmailDailyReportSuccess()).addAttachment(
                    "FEMA-Relatório_${currentDate.get(Calendar.YEAR)}-${currentDate.get(Calendar.MONTH)}-${
                        currentDate.get(Calendar.DAY_OF_MONTH)
                    }.pdf", report.readBytes(), "application/pdf"
                ).setTo(emails)
            ).onItem().invoke { _ -> logger.info("Successfully sent email") }.onFailure()
