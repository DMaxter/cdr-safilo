...
class FemaClient {
    ...
    @Inject @ConfigProperty(name = "fema.username") lateinit var authUser: String

    @Inject @ConfigProperty(name = "fema.password") lateinit var authPass: String

    @Inject @RestClient lateinit var expeditionsClient: FemaExpeditions
    ...
    fun getServices(source: Address, destination: Address): Uni<List<Service>> {
        ...
        return expeditionsClient.getServices(body).onItem().transform { response ->
            val doc: SOAPMessage =
                    xmlBuilder.createMessage(null, response.toByteArray().inputStream())

            val soapBody = doc.soapBody.extractContentAsDocument()

            val state =
                    getXMLElement(soapBody, "ServicosResponse/ServicosResult/State")?.textContent
            val message =
                    getXMLElement(soapBody, "ServicosResponse/ServicosResult/Message")?.textContent
            val services =
                    getXMLElement(soapBody, "ServicosResponse/ServicosResult/Servicos")?.childNodes
    ...
    fun openWaybill(waybill: WaybillDto): Uni<Label> {
        ...
        return expeditionsClient.openWaybill(body.replace("&", "&amp;")).onItem().transform {
                response ->
            val doc: SOAPMessage =
                    xmlBuilder.createMessage(null, response.toByteArray().inputStream())
            ...
            val id =
                    getXMLElement(soapBody, "CriarResponse/CriarResult/Numero")
                            ?.textContent
                            ?.toLongOrNull()
            val pdf =
                    getXMLElement(soapBody, "CriarResponse/CriarResult/Etiqueta/base64Label")
                            ?.textContent
    ...
    fun cancelWaybill(waybillId: Long): Uni<Void> {
        ...
        return expeditionsClient.cancelWaybill(body).onItem().transformToUni { response ->
            ...
            val message =
                    getXMLElement(soapBody, "AnularResponse/AnularResult/Message")?.textContent
    ...
    fun getWaybill(waybillId: Long, labelFormat: String): Uni<File> {
        ...
        return expeditionsClient.getWaybill(body).onItem().transform { response ->
            ...
            val state =
                    getXMLElement(soapBody, "EtiquetaResponse/EtiquetaResult/State")?.textContent
            val message =
                    getXMLElement(soapBody, "EtiquetaResponse/EtiquetaResult/Message")?.textContent
            val pdf =
                    getXMLElement(soapBody, "EtiquetaResponse/EtiquetaResult/Etiqueta/base64Label")
                            ?.textContent
    ...
    fun closeDay(): Uni<File> {
        ...
        return expeditionsClient.closeDay(body).onItem().transform { response ->
            ...
            val state =
                    getXMLElement(soapBody, "FecharDiaResponse/FecharDiaResult/State")?.textContent
            val message =
                    getXMLElement(soapBody, "FecharDiaResponse/FecharDiaResult/Message")
                            ?.textContent
            val pdf =
                    getXMLElement(soapBody, "FecharDiaResponse/FecharDiaResult/Report/base64Label")
                            ?.textContent
