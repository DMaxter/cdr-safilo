          ...
          <v-card elevation="12" :height="$vuetify.breakpoint.height > 620
            ...
            <v-row no-gutters justify="start" class="pt-2 pl-2">
              <template v-if="this.menu1">
                <v-menu :offset-x="true" tile>
                  <template v-slot:activator="{ on, attrs }">
                    <v-btn height="64" width="100" class="white--text customGradient" tile color="#808080"
                  ...
                  <v-btn-toggle v-model="icon" tile dark borderless>
                    <v-btn color="#808080" value="left" @click="$router.push('profile')" height="64" width="160"
                    ...
                    <v-btn color="#808080" value="center1" @click="$router.push('history')" height="64" width="160"
                      class="customGradient">
                    ...
                    <v-btn color="#808080" value="center2" @click="$router.push('search')" height="64" width="160"
                    ...
                    <v-btn color="#808080" value="right" @click="$router.push('orderClient')" height="64" width="160"
                      class="customGradient">
              ...
              <template v-else-if="this.menu2">
                <v-menu :offset-x="true" tile>
                  <template v-slot:activator="{ on, attrs }">
                    <v-btn height="64" width="100" tile class="white--text customGradient" color="#6e4e5d"
                      v-bind="attrs" v-on="on">
                      Menu
                    </v-btn>
                  </template>
                  ...
                  <v-btn-toggle v-model="icon" dark borderless tile>
                    <v-btn color="#6e4e5d" value="left" height="64" @click="$router.push('profile')" width="160"
                      class="customGradient">
                      <span class="white--text" style="font-size: 12px">Perfil</span>
                    ...
                    <v-btn color="#6e4e5d" value="center2" @click="$router.push('notes')" height="64" width="160"
                      class="customGradient">
                    ...
                    <v-btn color="#6e4e5d" value="right" @click="$router.push('materiais')" height="64" width="160"
                      class="customGradient">
            ...
            <v-row justify="center" align="center" class="fill-height d-flex flex-column" :style="$vuetify.breakpoint.height > 620
              ? 'height: 480px'
              : 'height: 350px'">
              <v-dialog max-width="800" v-model="this.cancelShow" id="cancelStatus">
                <template>
                  <v-card>
                    <v-card-title class="justify-center"><span>Cancelamento de pedido</span></v-card-title>
                    <v-card-text v-show="this.cancelSuccess">
                      <span class="text-h5">
                        Pedido cancelado com sucesso!
                      </span>
                    </v-card-text>
                    <v-card-text v-show="this.cancelError">
                      <span class="text-h5">
                        Erro ao cancelar pedido {{ this.cancelError }}
                      </span>
                    </v-card-text>
                    <v-card-actions>
                      <v-btn text class="mr-2" @click="cancelShow = false">Voltar</v-btn>
                    </v-card-actions>
                  </v-card>
                </template>
              </v-dialog>
              <v-dialog v-model="nota" transition="dialog-bottom-transition" max-width="800" id="dialogo">
                <template v-if="show">
                  <v-card>
                    <v-card-title class="justify-center"> <span v-show="!added && !failed"> {{ this.currClient.name }};
                        {{ this.currClient.address }}; {{ this.currClient.postalCode }} </span></v-card-title>
                    <v-card-text>
                      <div class="text-h6 mb-5" v-show="!added && !failed">Notas:</div>
                      <v-textarea tile outlined style="pointer-events: none;" hide-details auto-grow
                        v-model="noteToAdd"></v-textarea>
                    </v-card-text>
                    <v-card-actions class="justify-end">
                      <v-btn text class="mr-2" @click="nota = false">Voltar</v-btn>
                    </v-card-actions>
                  </v-card>
                </template>
              </v-dialog>
              <h3> PEDIDO {{ store.pedidoAtual.cod }} - CÓD. CLIENTE
                <span v-if="show" style="color: #e361b0" @click="dialog5 = true"> <u> {{ store.pedidoAtual.codClient }}
                  </u></span>
              ...
              <v-card elevation="12" color="#FAFAFA" :height="$vuetify.breakpoint.height > 620
                ...
                <template v-if="$vuetify.breakpoint.height > 620">
                  ...
                  <v-row justify="space-between" align="start" class="d-flex flex-column fill-height mt-0">
                    ...
                    <v-slide-group multiple show-arrows :class="{ 'ml-2': theme }">
                      <v-slide-item v-for="n in store.pedidoAtual.material.length" :key="n" v-slot="{ active, toggle }">
                        <v-btn :input-value="active" active-class="purple white--text" depressed class="ml-5 mr-5" dense
                          @click="toggle">
                    ...
                    <v-slide-group multiple show-arrows :class="{ 'ml-2': theme }">
                      <v-slide-item v-for="n in store.finishes.length" :key="n" v-slot="{ active, toggle }">
                        <v-btn :input-value="active" active-class="purple white--text" depressed class="ml-5 mr-5" dense
                          @click="toggle">
                          {{ store.finishes[n - 1] }}
                        </v-btn>
                    ...
                    <v-slide-group multiple show-arrows :class="{ 'ml-2': theme }">
                      <v-slide-item v-for="n in store.pedidoAtual.dimensoes.length" :key="n"
                ...
                <template v-else>
                  <v-row justify="space-between" align="start" class="d-flex flex-column mt-0">
                    ...
                    <v-col cols="auto" class="ml-4"> Material:</v-col>
                    <v-slide-group multiple show-arrows :class="{ 'ml-4': theme }">
                      <v-slide-item v-for="n in store.pedidoAtual.material.length" :key="n" v-slot="{ active, toggle }">
                        <v-btn :input-value="active" active-class="purple white--text" depressed class="ml-5 mr-5" dense
                          @click="toggle">
                          {{ store.pedidoAtual.material[n - 1] }}
            ...
            <v-row no-gutters align="end" justify="space-between" class="d-flex mt-4" :style="$vuetify.breakpoint.height > 620
              ...
              <v-col cols="auto" class="pl-4">
                <v-btn @click="goBack()" class="d-flex flex-column customGradient" small tile dark> <v-icon
                    style="transform: rotate(180deg);">mdi-play</v-icon>
                  Voltar
                </v-btn>
              </v-col>
              <v-col cols="auto" class="ml-4 d-flex">
                <v-btn class="mr-2 d-flex flex-column customGradient" small tile dark @click="print()">
                  <v-icon>mdi-printer</v-icon>
                </v-btn>
                <v-btn aria-label="Cancelar" class="mr-2 d-flex, flex-column customGradient" small tile dark
                  @click="cancel">
                ...
                <v-btn v-if="!this.hasWaybill && this.show" class="ml-2 d-flex flex-column customGradient" small tile
                  dark @click="added = false, failed = false, dialog2 = true;"> <v-icon>mdi-truck-delivery</v-icon>
                </v-btn>
                <v-btn v-if="this.hasWaybill && this.show" class="mr-1 d-flex flex-column customGradient" small tile
                  dark @click="added = false, failed = false, dialog3 = true;"> <v-icon>mdi-download</v-icon>
                </v-btn>
                <v-btn v-if="this.hasWaybill && this.show" class=" ml-1 d-flex flex-column customGradient" small tile
                  dark @click="added = false, failed = false, dialog4 = true;"> <v-icon>mdi-close</v-icon>
                </v-btn>
              </v-col>
              <v-col cols="auto" class="pr-4">
                <v-btn class="d-flex flex-column customGradient" small tile dark @click="getPedido()"> Ver Pedido
        ...
        <form @submit.prevent="waybill">
          <v-card tile>
            <v-card-title class="d-flex flex-column justify-center pt-3 pb-0">
              ...
              <span style="font-size: 12px" v-show="!added && !failed && !displayLoad"> Campos que começem com * são
                obrigatórios! </span>
            </v-card-title>
            <v-card-text class="pb-0">
              <v-container>
                <v-row justify="center">
                  ...
                  <span class="text-h5" v-show="failed">Ocorreu um erro:<br />{{ this.errorMsg
                    }}
                  </span>
                  ...
                  <v-col cols="6" v-show="!added && !failed && !displayLoad">
                    <v-autocomplete clearable hide-details label="*Serviço" :items="allServices" item-value="id"
                      item-text="name" required v-model="pickedService"></v-autocomplete>
                  </v-col>
                  <v-col cols="6" v-show="!added && !failed && !displayLoad">
                    <v-text-field hide-details type="number" label="*Número de pacotes" min=0 required
                      v-model="numberOfPackages"></v-text-field>
                  </v-col>
                  <v-col cols="6" v-show="!added && !failed && !displayLoad">
                    <v-autocomplete clearable hide-details label="*Tipo de encomenda" :items="packageTypes"
                      item-value="id" item-text="name" required v-model="pickedPackage"></v-autocomplete>
                  </v-col>
                  <v-col cols="6" v-show="!added && !failed && !displayLoad">
                    <v-text-field hide-details type="number" label="*Peso Total (Kg)" min=0 step=0.01 required
                      v-model="totalWeight"></v-text-field>
                  </v-col>
                  <v-col cols="12" v-show="!added && !failed && !displayLoad">
                    <v-text-field hide-details label="*Descrição" required v-model="description"></v-text-field>
                  </v-col>
                  <v-col cols="3" v-show="!added && !failed && !displayLoad">
                    <v-autocomplete clearable hide-details label="*Formato" :items="allFormats" item-value="id"
                      item-text="name" required v-model="pickedFormat"></v-autocomplete>
                  </v-col>
                  <v-col cols="3" v-show="!added && !failed && !displayLoad">
                  ...
                  <v-col cols="4" v-show="!added && !failed && !displayLoad">
                    <v-text-field hide-details label="Nr. de Telefone" v-model="phoneNumber"></v-text-field>
                  </v-col>
                  <v-col cols="4" v-show="!added && !failed && !displayLoad">
                    <v-autocomplete clearable hide-details label="País" v-model="country"
                      :items="paises"></v-autocomplete>
                  </v-col>
                  <v-col cols="4" v-show="!added && !failed && !displayLoad">
                    <v-text-field hide-details label="Cidade" v-model="city"></v-text-field>
                  </v-col>
                  <v-col cols="6" v-show="!added && !failed && !displayLoad" class="pb-3">
                    <v-text-field hide-details label="Morada" v-model="address"></v-text-field>
                  </v-col>
                  <v-col cols="6" v-show="!added && !failed && !displayLoad" class="pb-3">
                    <v-text-field hide-details label="Cód. Postal" v-model="postalCode"></v-text-field>
                  </v-col>
                </v-row>
              </v-container>
            </v-card-text>
      ...
      <v-dialog v-if="this.show" v-model="dialog3" persistent content-class="rounded-0" max-width="600px">
        <v-card tile>
          ...
          <v-card-text>
            <v-container>
              <v-row justify="center">
                ...
                <span class="text-h5" v-show="failed">Ocorreu um erro a descarregar a carta de porte:<br />{{
                  this.errorMsg }}
                </span>
                <v-progress-circular indeterminate color="grey" v-show="displayLoad"></v-progress-circular>
                <v-col cols="6" v-show="!added && !failed && !displayLoad">
                  <v-select hide-details label="Formato" :items="allFormats" item-value="id" item-text="name" required
                    v-model="pickedFormat"></v-select>
                </v-col>
              </v-row>
            </v-container>
          </v-card-text>
          <v-card-actions class="justify-center">
            ...
            <v-btn color="blue darken-1" text v-show="!added && !failed && !displayLoad"
              @click="downloadWaybill(); pickedFormat = null;">
              Descarregar
            </v-btn>
          </v-card-actions>
        </v-card>
          ...
          <v-card-text>
            <v-container>
              <v-row justify="center">
                ...
                <span class="text-h5" v-show="failed">Ocorreu um erro ao remover a carta de porte:<br />{{ this.errorMsg
                  }}</span>
                <v-progress-circular indeterminate color="grey" v-show="displayLoad"></v-progress-circular>
                <v-col cols="12" v-show="!added && !failed && !displayLoad">
      ...
      <v-dialog v-if="this.show" v-model="dialog5" content-class="rounded-0" max-width="600px">
        <v-card tile>
          ...
          <v-card-text class="pb-0">
            <v-container>
              <v-row justify="center">
                <v-col cols="12"> <span class="text-h6"> Nome: </span> <span class="text-body-1"> {{ currClient.name }}
                  </span></v-col>
                <v-col cols="12"> <span class="text-h6"> Email: </span> <span class="text-body-1">{{ currClient.email
                    }}</span></v-col>
                <v-col cols="12"> <span class="text-h6"> Telefone: </span> <span class="text-body-1">{{ currClient.phone
                    }}</span></v-col>
                <v-col cols="12"> <span class="text-h6"> Morada: </span> <span class="text-body-1">
                    {{ currClient.address }}</span></v-col>
                <v-col cols="12"> <span class="text-h6"> Cód. Postal: </span> <span class="text-body-1">{{
                  currClient.postalCode }}</span></v-col>
              </v-row>
            </v-container>
          </v-card-text>
          <v-card-actions class="justify-center">
          ...
          <v-col cols="auto">
            <v-card elevation="12" height="920" width="785" tile style="background-color: #E0E0E0; bottom: 40px;">
              <v-row justify="center" align="center" class="fill-height d-flex flex-column" style="height: 250px">
                <h3 class="mb-6 mt-6"> PEDIDO {{ store.pedidoAtual.cod }} - {{ store.pedidoAtual.modelo }}, Cliente: {{
                  store.pedidoAtual.nameClient }}, Cód: {{ store.pedidoAtual.codClient }}</h3>
...
export default {
  ...
  methods: {
    ...
    async cancel() {
      try {
        await Backend.cancelRequest(store.pedidoAtual.cod);
